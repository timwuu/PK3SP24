...
#include <stdbool.h>
#include <uart.h>   //timwuu 2015.04.11 change from lcd.h to uart.h

/* Type Definitions *************************************************/
typedef enum
{
    PRINT_CONFIGURATION_UART   //timwuu 2015.04.11 change from PRINT_CONFIGURATION_LCD
} PRINT_CONFIGURATION;
...
#define PRINT_SetConfiguration(configuration) UART_Initialize()
...
#define PRINT_String(string, length) UART_PutString(string, length)

void PRINT_Bytes(char* string, uint16_t length);
#define PRINT_Bytes(string, length) UART_PutBytes(string, length)
...
#define PRINT_ClearScreen() UART_ClearScreen()
