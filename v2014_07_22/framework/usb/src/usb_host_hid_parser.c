...
#ifdef DEBUG_MODE
    UART2PrintString("\nHID: Memory for Report Descriptor: ");
...
#ifdef DEBUG_MODE
    UART2PrintString("UPDATE VIRTUAL COLLECTION!\n");
#endif
...
void USBHID_ReportDecriptor_Dump(void) {
    ...
    for (i = 0; i < deviceRptInfo.collections; i++) {
        ...
        lcollection = &itemListPtrs.collectionList[i];
        UART2PrintString("\r\n    Data            : ");
        UART2PutHex32(lcollection->data);
        UART2PrintString("\r\n    Usage Page      : ");
        UART2PutHex16(lcollection->usagePage);
        UART2PrintString("\r\n    1st Usage Item  : ");
    ...
    for (i = 0; i < deviceRptInfo.reports; i++) {
        ...
        UART2PrintString("\r\nReportID : ");
        UART2PutHex16(lreport->reportID);
        if (lreport->inputBits > 0) {
            UART2PrintString("\r\nInbits   : ");
            UART2PutHex16(lreport->inputBits);
        }
        if (lreport->outputBits > 0) {
            UART2PrintString("\r\nOutbits  : ");
            UART2PutHex16(lreport->outputBits);
        }
        if (lreport->featureBits > 0) {
            UART2PrintString("\r\nFeatbits : ");
            UART2PutHex16(lreport->featureBits);
        }
    }
    for (i = 0; i < deviceRptInfo.reportItems; i++) {
        ...
        UART2PrintString("\r\nReportType  : ");
        UART2PutHex(lreportItem->reportType);
        //    HIDGlobals            globals;
        UART2PrintString("\r\nStart Bit   : ");
        UART2PutHex(lreportItem->startBit);
        UART2PrintString("\r\nBits        : ");
        UART2PutHex(lreportItem->globals.reportsize);
        UART2PrintString("\r\nParent      : ");
        UART2PutHex(lreportItem->parent);
        UART2PrintString("\r\nDataModes   : ");
        UART2PutHex32(lreportItem->dataModes);
        UART2PrintString("\r\n1st Usage   : ");
        UART2PutHex(lreportItem->firstUsageItem);
        UART2PrintString("\r\nUsage Items : ");
        UART2PutHex(lreportItem->usageItems);
        UART2PrintString("\r\n      Globals   ");
        UART2PrintString("\r\nusagePage   : ");
        UART2PutHex16(lreportItem->globals.usagePage);
        UART2PrintString("\r\nreportsize   : ");
        UART2PutHex(lreportItem->globals.reportsize);
        UART2PrintString("\r\nreportID   : ");
        UART2PutHex(lreportItem->globals.reportID);
        UART2PrintString("\r\nreportCount   : ");
        UART2PutHex(lreportItem->globals.reportCount);
    ...
    for (i = 0; i < deviceRptInfo.usageItems; i++) {
        ...
        if (itemListPtrs.usageItemList[i].isRange) {
            ...
            UART2PrintString("\r\nUsage: ");
            UART2PutHex16(itemListPtrs.usageItemList[i].usage);
        }
    }
