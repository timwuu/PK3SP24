...
#ifdef DEBUG_ENABLE
    #include "uart.h"
    #define DEBUG_PutString(str) UART2PrintString(str)
    #define DEBUG_PutChar(c) UART2PutChar(c)
    #define DEBUG_PutHexUINT8(uint8) UART2PutHex(uint8)
#endif
...
#ifdef DEBUG_MODE   //timijk
           UART2PrintString("EOFErr:");
#endif
            ...
            uint32_t *byteCount )
{
    ...
    if (ep != NULL)
    {
        ...
        if (ep->status.bfTransferSuccessful)
        {
            ...
            *byteCount = ep->dataCount;  //timijk #dataCount #byteCount
        }
...
//    }
    ...
    {    
        ...
        DEBUG_PutString( "HOST: usbDeviceInfo.currentConfiguration: ");
        DEBUG_PutHexUINT8(usbDeviceInfo.currentConfiguration);
        DEBUG_PutChar('\n');
#endif
...
#ifdef DEBUG_MODE
                if (pCurrentEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_INTERRUPT) {
                    UART2PutChar('#');
                    UART2PutHex(packetSize);   //timijk #dataCount #byteCount
                }
#endif                 
                ...
                switch( pCurrentEndpoint->bmAttributes.bfTransferType )
                {
                    ...
                    case USB_TRANSFER_TYPE_INTERRUPT:
                        if ((pCurrentEndpoint->bEndpointAddress & 0x80) == 0x00)
                        {
                            ...
                            {
#ifdef DEBUG_MODE   //timijk
                                UART2PrintString("ENDPOINT_NAK_TIMEOUT:");
#endif
